...
const validatePassword = (password) => {
  // 8-18 chars, at least 1 upper, 1 lower, 1 number
...
const handleLogin = async () => {
  ...
  setTimeout(() => {
    authStore.login({
      name: formData.value.email.split('@')[0], // Use part of email as name
      email: formData.value.email,
    })
    notificationStore.addNotification('Xush kelibsiz!', 'success')
    router.push('/')
  ...
  <div class="min-h-[80vh] flex items-center justify-center px-4">
    <div class="w-full max-w-md bg-white dark:bg-slate-900 rounded-[40px] p-10 shadow-float border border-gray-50 dark:border-slate-800">
      <div class="text-center mb-10">
        ...
        <h1 class="text-3xl font-black text-slate-900 dark:text-white">Xush kelibsiz!</h1>
        <p class="text-slate-500 mt-2 font-medium">Tizimga kirish uchun ma'lumotlarni kiriting</p>
      </div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        <div class="flex justify-end">
          <a href="#" class="text-sm font-bold text-orange-500 hover:underline">Parolni unutdingizmi?</a>
        </div>
      ...
      <div class="mt-10 text-center">
        <p class="text-slate-500 font-medium">
          ...
          <RouterLink to="/signup" class="text-orange-500 font-black hover:underline ml-1">Ro'yxatdan o'tish</RouterLink>
        </p>
      </div>
    </div>
  </div>
